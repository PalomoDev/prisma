...
export async function removeEntireItemFromCart(productId: string) {
    try {
        // Check for cart cookie
        const sessionCartId = (await cookies()).get('sessionCartId')?.value;
        if (!sessionCartId) throw new Error('Cart session not found');
        ...
        // Revalidate paths
        revalidatePath(`/product/${product.slug}`);
        revalidatePath('/cart'); // Also revalidate cart page since we're completely removing an item

        return {
            success: true,
            message: `${product.name} was completely removed from cart`,
        };
    } catch (error) {
        return { success: false, message: formatError(error) };
    }
