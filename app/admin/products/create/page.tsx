...
const CreateProductPage = async () => {
    ...
    try {
        ...
        const [categories, subcategories, brands, features, specifications] = await Promise.all([
            getCategories(true),
        ...
        console.log('📋 Categories result:', {
            ...
            data: categories.data?.map(cat => ({ id: cat.id, name: cat.name, slug: cat.slug }))
        });

        console.log('📋 Subcategories result:', {
            success: subcategories.success,
            count: subcategories.data?.length || 0,
            data: subcategories.data?.map(sub => ({ id: sub.id, name: sub.name, slug: sub.slug }))
        });

        console.log('🏷️ Brands result:', {
            success: brands.success,
            count: brands.data?.length || 0,
            data: brands.data?.map(brand => ({ id: brand.id, name: brand.name, slug: brand.slug }))
        });
        ...
        console.error('💥 CreateProductPage: Критическая ошибка при загрузке данных:', error);
