...
export default async function EditProductPage({
  ...
  try {
    ...
    const safeSpecifications =
      specificationsResult.status === "fulfilled"
    ...
    if (categoriesResult.status === "rejected")
      errors.push("Не удалось загрузить категории");
    if (subcategoriesResult.status === "rejected")
      errors.push("Не удалось загрузить подкатегории");
    if (brandsResult.status === "rejected")
      errors.push("Не удалось загрузить бренды");
    if (featuresResult.status === "rejected")
      errors.push("Не удалось загрузить особенности");
    if (specificationsResult.status === "rejected")
      errors.push("Не удалось загрузить характеристики");

    // Сериализуем данные
    const product = prismaToJson(rawProduct);
    const serializedCategories = prismaToJson(
      safeCategories
    ) as typeof safeCategories;
    const serializedSubcategories = prismaToJson(
      safeSubcategories
    ) as typeof safeSubcategories;
    const serializedBrands = prismaToJson(safeBrands) as typeof safeBrands;
    const serializedFeatures = prismaToJson(
      safeFeatures
    ) as typeof safeFeatures;
    const serializedSpecifications = prismaToJson(
      safeSpecifications
    ) as typeof safeSpecifications;
