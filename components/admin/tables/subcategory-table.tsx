...
import {GetSubcategoriesResponse} from "@/types";
        ...
        <Table className={'mt-4 border'}>
            <TableHeader>
                <TableRow>
                    ...
                    <TableHead className='border-r text-center w-[187px]'>SUBCATEGORY</TableHead>
            ...
            <TableBody>
                {subCategories.data.map((subcategory) => {
                    const availableCategories = subcategory.categorySubcategories
                        ?.map(cs => cs.category.name) || [];

                    return (
                        <TableRow key={subcategory.id}>
                            <TableCell className='border-r border-l text-center'>{formatId(subcategory.id)}</TableCell>
                            <TableCell className='border-r border-l text-center'>{subcategory.name}</TableCell>
                            <TableCell className='border-r border-l text-center'>{subcategory.slug}</TableCell>
                            <TableCell className='border-r border-l text-center'>{subcategory.description || '–'}</TableCell>
                            <TableCell className='border-r border-l text-center'>{subcategory.image || '–'}</TableCell>
                            ...
                            <TableCell className='border-r border-l text-center'>{subcategory._count.productSubcategories}</TableCell>
                            <TableCell className='border-r border-l text-center'>{subcategory.sortOrder}</TableCell>
                            <TableCell className='border-r border-l text-center'>
                            ...
                            <TableCell className='flex justify-center gap-1'>
                                <Button asChild variant='ghost' size='sm'>
                                    <Link href={`/admin/products/category/update-subcategory/${subcategory.id}`}>
